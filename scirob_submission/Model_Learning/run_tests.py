...
def run_eq_test(path_to_test_data,
             loaded_model,
             scaler,
             start_point: int,
             run_timespan: int,
    ...
    initial, _, _, vx, delta, fx = dataset_separation(input_shape, output_shape, input_timesteps,
                                                              data, start_point, run_timespan)
    ...
    for i_count in tqdm(range(0, len(delta))):
        ...
        result_process = np.array([yaw_rate, vy]).reshape(1, 2)

        if abs(result_process[:, 0]) > 100 or abs(result_process[:, 1]) > 1000:
            print(name + '[Error in computing the prediction. Values are too big!]')
            return 0, -1
        ...
        results[i_count + input_timesteps, 0:output_shape - 1] = result_process
        ...
        # Add prediction to the next input
        temp[:, input_shape * (input_timesteps - 1):input_shape * (input_timesteps - 1) + output_shape - 1] = result_process

        # Add control signals
        temp[:, input_shape * (input_timesteps - 1) + output_shape - 1] = vx[i_count]
        temp[:, input_shape * (input_timesteps - 1) + output_shape] = delta[i_count]
        temp[:, input_shape * (input_timesteps - 1) + output_shape + 1] = fx[i_count]
    ...
    results[:, output_shape-1:input_shape] = data[start_point:start_point + len(delta) + input_timesteps,
