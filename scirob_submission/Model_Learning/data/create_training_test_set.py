...
def create_training_set_car_perf(path_to_data, path_to_output, number_of_sets, step):
    sum_ = 0
    directories = ['train_car_perf/mid_high_perf/', 'train_car_perf/high_perf/']
    result = []
...
def create_training_set_road_grip(path_to_data, path_to_output, number_of_sets, step):
    sum_ = 0
    directories = ['train_road_grip/grip_06/', 'train_road_grip/grip_08/']
    result = []
    for i in range(number_of_sets*len(directories)):
        temp = []
        result.append(temp)
    ...
    print('SAVING ACCELERATIONS')
    np.savetxt(path_to_output + 'train_ax_' + str(step) + '_mu.csv', ax_to_plot, delimiter=',')
    np.savetxt(path_to_output + 'train_ay' + str(step) + '_mu.csv', ay_to_plot, delimiter=',')

    print('FLATTENING')
    # Put together all data extracted
    result_flat = flatten_extend(result)
    np.savetxt(path_to_output + 'train_data_step_flat_' + str(step) + '_mu.csv', result_flat, delimiter=',')
    ...
    # Remove all lists of all-zero elements
    result_filtered = result_flat[~np.all(result_flat == 0, axis=1)]
    np.savetxt(path_to_output + 'train_data_step_no_shuffle_' + str(step) + '_mu.csv', result_filtered, delimiter=',')

    print('SHUFFLING')
    # Shuffle input
    np.random.seed(1)
    np.random.shuffle(result_filtered)
    np.savetxt(path_to_output + 'train_data_step' + str(step) + '_mu.csv', result_filtered, delimiter=',')
