...
class Simulation:
	...
	def plotResults(self, xaxis = "s"):
		...
		FzF = self.logFile["FzF"]
		FzR = self.logFile["FzR"]
		...
		plt.figure()
		ax8 = plt.subplot(4, 1, 1, sharex = ax1)
		...
		ax9 = plt.subplot(4, 1, 2, sharex = ax1)
		...
		ax10 = plt.subplot(4, 1, 3, sharex = ax1)
		...
		ax13 = plt.subplot(4, 1, 4, sharex = ax1)
		ax13.plot(x, FzF / 1000, 'k', linewidth = 1)
		ax13.plot(x, FzR / 1000, 'r--',linewidth = 2)
		plt.legend(('Front','Rear'))
		plt.grid(True)
		plt.ylabel('Normal Forces (kN)')
...
class Logger:
	...
	def __init__(self, NUMBER_DATA_POINTS = 300000):
		self.data = {}
		self.counter = 0
...
class Replay:
	...
	def simControllerOutputs(self):
		...
		auxVars = {"betaFFW": np.zeros((N, 1)), "deltaFFW": np.zeros((N,1))}
		controlInput = ControlInput()
		...
		for i in range(N):
			...
			av = self.controller.updateInput(localState, controlInput)
			inputs["deltaCmd"][i] = controlInput.delta
			inputs["FxCmd"][i] = controlInput.Fx
			auxVars["betaFFW"][i] = av["betaFFW"]
			auxVars["deltaFFW"][i] = av["deltaFFW"]

		return inputs, auxVars
