...
def train_neuralnetwork(path_dict: dict,
    ...
    """
    data = src.load_data_for_nn.load_data(path2inputs_trainingdata=path_dict['path2inputs_trainingdata_CRT'],
                                          filename_trainingdata=path_dict['filename_trainingdata'])
    ...
    """
    print('TRAINING DATA')
    print('training samples length: ', len(train_data[0]), '\ntraining labels length: ', len(train_data[1]))
    for i in range(0, 5):
        print('Data: ', train_data[0][i], '\nLabel: ', train_data[1][i], '\n--------------------')
