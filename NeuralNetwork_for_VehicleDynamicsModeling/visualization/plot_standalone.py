...
def plot_run(filepath2results: str,
    ...
    ax_label = labels[start:1500 + start, 3][:, np.newaxis]
    ay_label = labels[start:1500 + start, 4][:, np.newaxis]
    ...
    plot_and_save(ay_result, ay_label, ay_diff, 'Lat. acc. ay in m/s2',
                  filepath2plots + 'ay' + str(counter) + '.png')
    plot_and_save(ax_result, ax_label, ax_diff, 'Long. acc. ax in m/s2',
                  filepath2plots + 'ax' + str(counter) + '.png')
...
def plot_run_test_CRT(filepath2results: str,
    ...
    # plot and save comparison between NN predicted and actual vehicle state
    plot_and_save(yaw_result, yaw_label, yaw_diff, 'Yaw rate [rad/s]',
    ...
    plot_and_save(ay_result, ay_label, ay_diff, 'Lat. acc. ay [m/s2]',
                  filepath2plots + 'ay_test_' + str(counter) + '.png')
    plot_and_save(ax_result, ax_label, ax_diff, 'Long. acc. ax [m/s2]',
...
def plot_and_save(inp_1,
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(20, 15), sharex='all')
    ...
    ax.plot(inp_1, label='Result', color='tab:orange')
    ax.plot(inp_2, label='Label', color='tab:blue')
    ax.set_ylabel(value)
    ax.set_xlabel('Time steps (8 ms)')
    ax.legend()

    index_last_underscore = savename.rfind('_')
    savename = savename[:index_last_underscore + 1]
    fig.savefig(savename + 'full.png', format='png')
    plt.close(fig)
...
path_to_results = '../outputs/2024_05_06/14_20_41/matfiles/prediction_result_recurrent_CRT_'
path_to_data = '../inputs/trainingdata/test_set_'
path_to_plots = '../outputs/2024_05_06/14_20_41/figures/'
