...
yaws_NN = np.empty((len(ramp_steering[0]),), dtype=object)
yaws_NN_2 = np.empty((len(ramp_steering[0]),), dtype=object)
yaws_NN_3 = np.empty((len(ramp_steering[0]),), dtype=object)
yaws_bicycle = np.empty((len(ramp_steering[0]),), dtype=object)
yaws_bicycle_vx_comp = np.empty((len(ramp_steering[0]),), dtype=object)
yaws_labels = np.empty((len(ramp_steering[0]),), dtype=object)


for u, speed in enumerate(speeds):
    for idx, value in enumerate(ramp_steering[u]):
        if 'highspeed' in speed:
            testname = 'results_test_rampsteer_fx100_' + value + '.csv'
        elif 'lowspeed' in speed:
            testname = 'results_test_rampsteer_fx25_' + value + '.csv'

        basepath_NN = 'scirob_submission/Model_Learning/results/step_1/callbacks/'
        # path2results_NN = basepath_NN + '2024_10_13/16_22_36/' + testname
        ...
        plt.plot(time_values, vx_result_bicycle_vx_comp, label=name_bike, color=color_dict['Green_pastel'], linestyle='dashed', linewidth=2.5)
        ...
        plt.grid(True)
        if 'highspeed' in speed:
            plt.savefig(savepath + '/ramp_steering'
        ...
        plt.plot(time_values, vy_result_bicycle, label=name_bike, color=color_dict['Saffron_pastel'],
                 linewidth=2.5)
        ...
        plt.plot(time_values, vy_label, label='Ground Truth', color=color_dict['Blue_pastel'], linestyle='dashed',
                 linewidth=1.5)
        ...
        ax1.plot(time_values, yaw_result_NN[:1000], label=name_NN, color=color_dict['Red_pastel'], linewidth=2.5)
        """ax1.plot(time_values, yaw_result_NN_2[:1000], label=name_NN_2, color=color_dict['Saffron_pastel'], linewidth=2.5)
        ax1.plot(time_values, yaw_result_NN_3[:1000], label=name_NN_3, color=color_dict['Green_pastel'], linewidth=2.5)"""
        ax1.plot(time_values, yaw_result_bicycle[:1000], color=color_dict['Saffron_pastel'], label=name_bike, linewidth=2.5)
        ax1.plot(time_values, yaw_result_bicycle_vx_comp[:1000], color=color_dict['Green_pastel'], label=name_bike_vx_comp,
                 linestyle='dashed', linewidth=2.5)
        ax1.plot(time_values, yaw_label[:1000], color=color_dict['Blue_pastel'], label='Ground Truth', linestyle='dashed', linewidth=2.5)
        ...
        ax2.plot(time_values, steer_quantity_NN[:1000], color=color_dict['Purple_pastel'], label='Steering Input', linewidth=2.5)
    ...
    metrics_bike_vx_comp_values = list(
        {'Median RMSE': median_rmse_bike_vx_comp, 'Median MAE': median_mae_bike_vx_comp}.values())
