...
savepath = '../test/test_post_20241020/perconfrontomodellibike+crt'

ramp_steering = ['0', '1', '2', '3', '4', '5']

yaws_NN = np.empty((len(ramp_steering),), dtype=object)
yaws_NN_2 = np.empty((len(ramp_steering),), dtype=object)
...
for idx, value in enumerate(ramp_steering):
    ...
    plt.plot(time_values, vx_result_NN, label='NN: Bike (μ=1) + CRT (μ=1)', color='r', linewidth=1.5)
    ...
    time_values = np.linspace(0, len(vx_result_NN) / 100, len(vx_result_NN))
    plt.plot(time_values, vy_result_NN, label='NN: Bike (μ=1) + CRT (μ=1)', color='r', linewidth=1.5)
    ...
    ax1.plot(time_values, yaw_result_NN[:1000], color='red', label='NN: Bike (μ=1) + CRT (μ=1)', linewidth=2.5)
...
plt.bar(x - 3/2*width, metrics_nn_values, width, label='NN: Bike (μ=1) + CRT (μ=1)', color='red')
...
plt.tight_layout()
plt.savefig(savepath + '/ramp_steering'
            '/metrics_comparison.png', format='png', dpi=300)
plt.close()
