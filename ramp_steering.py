...
for u, speed in enumerate(speeds):
    for idx, value in enumerate(ramp_steering[u]):
        ...
        time_values = np.linspace(0, len(vx_result_NN) / 100, len(vx_result_NN))
        plt.plot(time_values, vy_result_NN, label='NN: Bike (μ=1, μ=0.6) + CRT (μ=1)', color='r', linewidth=1.5)
        # plt.plot(time_values, vy_result_NN_2, label='NN: Bike (μ=1, μ=0.6) + CRT (μ=1, μ=0.6) errato', color='orange', linewidth=1.5)
        plt.plot(time_values, vy_result_bicycle, label='Bicycle model', color='green',
                 linewidth=1.5)
        ...
        ax1.plot(time_values, yaw_result_NN[:1000], color='red', label='NN: Bike (μ=1, μ=0.6) + CRT (μ=1)', linewidth=2.5)
        # ax1.plot(time_values, yaw_result_NN_2[:1000], color='orange', label='NN: Bike (μ=1, μ=0.6) + CRT (μ=1, μ=0.6) errato', linewidth=2.5)
        ax1.plot(time_values, yaw_result_bicycle[:1000], color='green', label='Bicycle model', linewidth=2.5)
        ax1.plot(time_values, yaw_result_bicycle_vx_comp[:1000], color='green', label='Bicycle model with Fx as input',
