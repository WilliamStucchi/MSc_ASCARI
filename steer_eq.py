...
for idx, value in enumerate(step_steering):
    testname = 'results_test_steereq_fx100_' + value + '.csv'
    ...
    ax1.set_xlabel('Time [10 ms steps]')
    ax1.set_ylabel('Yaw Rate [rad/s]', color='black')
    ax1.plot(range(10000, 10000 + len(yaw_result_NN[10000:])), yaw_result_NN[10000:], color='red', label='Yaw rate Neural Network', linewidth=1.5)
    ax1.plot(range(10000, 10000 + len(yaw_result_NN[10000:])), yaw_result_bicycle[10000:], color='blue', label='Yaw rate Bicycle model', linewidth=1.5)
    ax1.tick_params(axis='y', labelcolor='black')

    # Secondo asse y per lo sterzo
    ax2 = ax1.twinx()  # Condividi lo stesso asse x
    ax2.set_ylabel('Steering Input [rad]', color='green')
    ax2.plot(range(10000, 10000 + len(yaw_result_NN[10000:])), steer_quantity_NN[10000:], color='green', label='Steering Input', linewidth=1.5)
